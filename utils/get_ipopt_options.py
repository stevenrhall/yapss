...
Ipopt stays the authority. The table is used to **explain a refusal**: when Ipopt rejects an
option, YAPSS can say whether the value was outside the documented range (a mistake, so an
error) or inside it (so the build probably does not implement that option -- ``linear_solver
= "ma27"`` is documented and valid, and fails on any build without HSL -- which is a warning).

Run it from the repository root::

    python utils/get_ipopt_options.py --specs   # writes the spec module
...
def render(specs: dict[str, dict[str, object]], ref: str) -> str:
    ...
    lines = [
        ...
        "edit by hand.",
        "",
        "This table never rejects a value on its own. Ipopt validates every option it is given,",
        "and the build in use need not match the documentation this was scraped from, so Ipopt",
        "stays the authority. The table explains a refusal: whether the value was outside the",
        "documented range, or inside it -- in which case the build most likely does not provide",
        "that option. See ``solver.py``.",
        "",
        ...
        "",
        "The Ipopt library actually loaded may be a different version, so a message built from",
        "this table says which version it is quoting.",
        '"""',
        "",
...
def main() -> int:
    ...
    if args.specs:
        ...
        print("    # only works when read from the ipopt.opt options file, so not settable")
        for name in sorted(specs):
            prefix = "# " if specs[name].get("file_only") else ""
            print(f"    {prefix}{name}: {specs[name]['kind']}")
    return 0
